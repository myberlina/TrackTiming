<?php
  // Results_Info:  Hillclimb style with up to 2 splits and speed import, no reaction time, Green light => Button, Start Beam => Green, Split1 => Start, Split2 => Split
  ...
  $speed_max = -1;
  $speed_query = 'SELECT g.run, g.car_num, speed
		    FROM green_time AS g, finish_time AS f, speeds AS s
		   WHERE g.event = ' . $db->escapeString($evt) . '
		     AND f.event = ' . $db->escapeString($evt) . '
		     AND s.event = ' . $db->escapeString($evt) . '
		     AND g.run = f.run
		     AND g.car_num = f.car_num
		     AND g.time_t NOTNULL
		     AND g.time_t < s.time_t
		     AND s.time_t < (g.time_t - g.time_ms / 1000 + f.time_ms / 1000)';
  $speed_qry = $db->query($speed_query);
  while($row = $speed_qry->fetchArray()) {
    $speeds[$row["run"]][$row["car_num"]] = $row["speed"] / 10;
    if ($speed_max < $row["speed"] / 10) $speed_max = $row["speed"] / 10;
  }
...
<div style="float:right">
 ...
 <!--TITLE_BANNER--
  ...
  <table align="center" border="1" cellpadding="1">
   ...
   $empty_run = "<td></td>";
   if (isset($speeds)) {
       echo "<td align=\"right\">km/h</td>";
       $empty_run = "<td></td><td></td>";
   }
   $i=1;
   while(++$i <= $max_runs) {
   ...
   while($row = $results->fetchArray()) {
	 ...
	 $st = $st_ms[$row["car_num"]][$row["run"]];
         if ($best_st[$row["car_num"]] == $st)
           if ($purple_st == $st)
               printf("&nbsp;<strong style=\"color: purple\">$split_fmt</strong> ", $st);
           else
               printf("&nbsp;<strong>$split_fmt</strong> ", $st);
         else
           printf("&nbsp;$split_fmt ", $st);
       }
     ...
     echo "</td>";
     if (isset($speeds[$row["run"]][$row["car_num"]])) {
         if ($speed_max == $speeds[$row["run"]][$row["car_num"]])
             printf("<td style=\"color: purple\" align=\"right\">%5.1f</td>", $speeds[$row["run"]][$row["car_num"]]);
         else
