<?php
  ...
  if (count($_POST)>0) {
    ...
    if ((isset($_POST['submit']))&&('Fix' == $_POST['submit'])&&($row_id>0)) {
      if ($post_qry = $db->prepare("UPDATE split_time set car_num=:num WHERE rowid=:row AND event=:event AND run=:run")){
    ...
    if((isset($_POST['really-delete']))&&('Yes' == $_POST['really-delete'])&&($row_id>0)) {
      if ($post_qry = $db->prepare("DELETE FROM split_time WHERE event=:event AND run=:run AND car_num=:num AND rowid=:row")){
        $post_qry->bindValue(':event', intval($db->escapeString($evt)), SQLITE3_INTEGER);
        $post_qry->bindValue(':run', intval($db->escapeString($run)), SQLITE3_INTEGER);
        $post_qry->bindValue(':num', intval($db->escapeString($_POST["CarNum-$row_id"])), SQLITE3_INTEGER);
        $post_qry->bindValue(':row', intval($row_id), SQLITE3_INTEGER);
  ...
  if (isset($_GET['ET'])) {
    ...
    $time_colour='Start';
    $time_query=
	 'SELECT split_time.rowid, split_time.car_num, split_time.time_ms, split_time.time_ms - start_time.time_ms delta
	  FROM split_time
	  LEFT JOIN start_time ON start_time.event = split_time.event AND start_time.run = split_time.run AND start_time.car_num = split_time.car_num AND split_time.time_ms > start_time.time_ms
  ...
  if (isset($_GET['col_name']))
    $time_title = htmlspecialchars($_GET['col_name']);
...
?>
  ...
  var ws = new WebSocket('ws://'+location.host+'/ws/status/split/');
